...
x_vals=np.array([[x[0],x[3]] for x in iris.data])#width 
...
alpha=tf.constant([0.01])#alpha value=0.1 试试
...
[[a1],[a2]]=sess.run(A)
[[b]]=sess.run(b)#enumberate 序号0 1 2 3 4 5 6 ...值对应...
slope=-a2/a1
y_intercept=b/a1
...
plt.plot(train_accuracy,'k-',label='train_accuracy')
...
plt.plot(loss_vec,'k-')
plt.title('loss per genernation')
plt.xlabel('generation')
plt.ylabel('loss')
plt.show()
