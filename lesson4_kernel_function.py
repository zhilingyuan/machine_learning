# svm d（w'x+b）>1 or <-1
# soft d(w'x+b) >1-ep or <ep-1
# c 正则化参数的倒数
# w'f=0
# j(w) minimize
# w=adx
# sum(a)-aaddx'x maxmize
# maxmize（0，1-**）
...
(x_vals,y_vals)=datasets.make_circles(n_samples=500,factor=.5,noise=.1
                                      )#y_vals=0 or 1   #factor 两种各占多少？
y_vals=np.array([1 if y==1 else -1 for y in y_vals])#y_vals=-1,1
...
dist=tf.reduce_sum(tf.square(x_data),1)#沿着某个维度，方向的和 0 列 1 行
dist=tf.reshape(dist,[-1,1])#-1 infer shape value 
...
accuracy=tf.reduce_mean(tf.cast(tf.equal(tf.squeeze(prediction),
                                         tf.squeeze(y_target)),tf.float32))#squeeze 删除大小是1的维度 指代shape
