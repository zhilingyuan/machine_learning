...
for style_layer in style_layers:
    ...
    feats, height, width, channels = [x.value for x in layer.get_shape()]#get_shape 返回的是元组
                                                                        #for x in  ->[] 或 .as_list()
...
total_var_x = sess.run(tf.reduce_prod(image[:,1:,:,:].get_shape()))#对应维度乘
...
#这里的total variation 和图片的不同之处 在于 除以 total  做平均
