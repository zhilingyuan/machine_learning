#knn is for labeled information
...
x_vals=(x_vals-x_vals.min(0))/x_vals.ptp(0)#ptp 返回最大值最小值的差 0 axis 方向
...
distance=tf.reduce_sum(tf.abs(tf.subtract(x_data_train,
                    tf.expand_dims(x_data_test,1))),
                       reduction_indices=2)
#L2
#distance=tf.sqrt(tf.reduce_sum(tf.square(tf.sub(x_data_train,tf.expand_dims(x_data_test,1)))
#                              reduction_indices=1))

top_k_xvals,top_k_indices=tf.nn.top_k(tf.negative(distance),k=k)
#top_k return the maximum value and index
...
num_loops=int(np.ceil(len(x_vals_test)/batch_size))#1
...
bins=np.linspace(5,50,46)#46 better?
plt.hist(predictions,bins,alpha=0.5,label='prediction')
plt.hist(y_batch,bins,alpha=0.5,label='actual')
plt.title('histogram of predicter and actual values')
